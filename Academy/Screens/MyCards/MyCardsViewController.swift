...
class MyCardsViewController: UIViewController {
    ...
    var viewModel: MyCardsViewModelType!
    ...
    fileprivate func checkFilterModel() {
        if viewModel.filterModel == nil || viewModel.endDate?.isEmpty == true {
            nameLabel.text = "На даний момент у вас немає тренувань"
    ...
    private func setupTable() {
        tableView.registerNib(cellType: CardsTableViewCell.self)
    ...
    @IBAction func groupCardsButtonAction(_ sender: Any) {
        viewModel.route(to: .groupCards)
    }
...
extension MyCardsViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell: CardsTableViewCell = tableView.dequeCell(for: indexPath) else { return UITableViewCell() }
