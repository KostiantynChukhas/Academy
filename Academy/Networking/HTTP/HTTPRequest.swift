...
enum HeaderType: String {
    ...
    case accepType = "Accept-Type"
    case mockedResponseCode = "x-mock-response-code"
}
...
extension HTTPRequest {
    func failure(data: Data, urlResponse _: HTTPURLResponse) throws -> AcademyAPIError {
        return try JSONDecoder().decode(AcademyAPIError.self, from: data)
    }
}
...
extension HTTPRequest {
    ...
    func buildURLRequest(_ auth: HTTPRequestAuth?) throws -> URLRequest {
        let url = path.isEmpty ? baseURL : URL(string: baseURL.absoluteString.appending(path))!
        ...
        if let auth = auth {
            urlRequest.setValue(auth.value, forHTTPHeaderField: auth.key)
        }
